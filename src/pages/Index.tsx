        ...
        <section className="relative min-h-[calc(100svh-3rem)] sm:min-h-[calc(100svh-3.5rem)] flex flex-col overflow-hidden">
          ...
          <div className="relative z-10 w-full px-5 sm:px-6">
            <div className="max-w-2xl mx-auto text-center flex flex-col items-center">
              ...
              {/* Subtitle */}
              <p className="text-sm sm:text-base text-muted-foreground max-w-sm sm:max-w-md mx-auto mb-4 leading-relaxed line-clamp-2 animate-fade-up opacity-0 stagger-2" style={{ animationFillMode: 'forwards' }}>
          ...
          {/* Bottom spacer — smaller so tiles sit lower */}
          <div className="flex-[2]" />

          {/* Athlete tiles — anchored toward bottom */}
          <div className="relative z-10 w-full px-4 sm:px-6 animate-fade-up opacity-0 stagger-4" style={{ animationFillMode: 'forwards' }}>
            <div className="max-w-lg mx-auto">
              <div className="flex gap-2 overflow-x-auto scrollbar-none snap-x snap-mandatory justify-center pb-1">
                {fomoFaces.map((athlete) => (
                  <Link
                    key={athlete.id}
                    to={`/athlete/${athlete.id}`}
                    className="shrink-0 snap-start w-[60px] h-[80px] sm:w-[68px] sm:h-[90px] rounded-xl overflow-hidden border border-border/30 transition-all duration-200 hover:border-primary/40 hover:shadow-[0_0_12px_hsl(var(--primary)/0.25)] active:shadow-[0_0_16px_hsl(var(--primary)/0.35)] active:scale-[0.97]"
                  >
                    <img
                      src={athlete.avatar}
                      alt={athlete.name}
