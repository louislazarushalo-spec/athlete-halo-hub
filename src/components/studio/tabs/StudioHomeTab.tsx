...
export const StudioHomeTab = ({ onNavigate }: StudioHomeTabProps) => {
  const [bioEditing, setBioEditing] = useState(false);
  const [bio, setBio] = useState("Game-changer on and off the pitch. Step inside my world.");
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="relative rounded-lg overflow-hidden bg-muted h-32 md:h-40 mb-12">
          ...
          <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between p-4">
            <div className="flex items-end gap-3">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-muted-foreground/20 border-4 border-background flex items-center justify-center text-2xl">
                👤
              </div>
              <div className="pb-1">
                <h2 className="font-semibold text-foreground text-lg">Your Name</h2>
              </div>
            </div>
          </div>
        </div>
        <div className="flex gap-2 mt-2">
          <Button variant="outline" size="sm" className="h-9">Update banner</Button>
          <Button variant="outline" size="sm" className="h-9">Update avatar</Button>
        </div>
      </StudioCard>
      ...
      <StudioCard
        ...
        onCtaClick={() => { setBioEditing(true); setBioTemp(bio); }}
      >
        {bioEditing ? (
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" className="h-9" onClick={() => { setBio(bioTemp); setBioEditing(false); }}>Save</Button>
            </div>
          </div>
        ) : (
      ...
      <StudioCard
        ...
        subtitle="All your media assets in one place."
          ...
          <div className="rounded-lg border border-border/50 p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                onClick={() => setExpandedSource(expandedSource === "social" ? null : "social")}
              >
            ...
            <div className="flex flex-wrap gap-1.5">
              {SOCIAL_SOURCES.map((s) => (
                <Badge key={s.name} variant="secondary" className={cn("text-xs font-normal", s.color)}>
                  {s.name}
                </Badge>
              ))}
            </div>
            {expandedSource === "social" && (
              <p className="text-xs text-muted-foreground mt-3 p-2 bg-muted/50 rounded">
                Connect your social accounts to auto-import content. Coming soon.
              </p>
            )}
          </div>
          ...
          <div className="rounded-lg border border-border/50 p-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <Button
                ...
                size="sm"
                className="h-8 text-xs"
                onClick={() => setExpandedSource(expandedSource === "owned" ? null : "owned")}
              >
            ...
            {expandedSource === "owned" && (
              <p className="text-xs text-muted-foreground mt-3 p-2 bg-muted/50 rounded">
                Add your federation or club page URLs. Coming soon.
              </p>
            )}
          </div>
          ...
          <div className="rounded-lg border border-border/50 p-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <Button
                ...
                size="sm"
                className="h-8 text-xs"
                onClick={() => setExpandedSource(expandedSource === "earned" ? null : "earned")}
              >
            ...
            {expandedSource === "earned" && (
              <p className="text-xs text-muted-foreground mt-3 p-2 bg-muted/50 rounded">
                Add media outlets and fan blog URLs. Coming soon.
              </p>
            )}
          </div>
        </div>
      ...
      <StudioCard
        title="Planning"
        subtitle="This week's priorities and what to do next."
      >
      ...
      {/* Growth snapshot */}
      <StudioCard
        title="Growth snapshot (7D)"
        subtitle="Your key metrics at a glance."
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Views", value: "12.4K", change: "+8%" },
            { label: "Opt-ins", value: "342", change: "+12%" },
            { label: "Participation", value: "18%", change: "+3%" },
            { label: "Revenue", value: "€1,240", change: "+5%" },
          ].map((kpi) => (
            <div key={kpi.label} className="rounded-lg bg-muted/40 p-3 text-center">
              <p className="text-lg font-semibold">{kpi.value}</p>
              <p className="text-xs text-muted-foreground">{kpi.label}</p>
              <p className="text-xs text-emerald-500 dark:text-emerald-400 mt-0.5">{kpi.change}</p>
            </div>
          ))}
        </div>
      </StudioCard>
